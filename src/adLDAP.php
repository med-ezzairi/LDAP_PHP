...
class adLDAP {
    ...
    /**
    *defined by zairi
    *const for grouptype to determin the type and the scope
    */
    const LOCAL_GROUP=-2147483644;
    const GLOBAL_GROUP=-2147483646;
    const UNIVERSEL_GROUP=-2147483640;
    ...
    * @var string
    */   
    protected $accountSuffix = "@techdev.net";
	//protected $accountSuffix = '@'.$domainName;
    ...
    * @var string
    */
    //protected $baseDn = "DC=moda,DC=com"; 
	protected $baseDn = "DC=techdev,DC=net"; 
    ...
    * @var array
    */
    //protected $domainControllers = array("144.145.30.200");
    protected $domainControllers = array("144.145.30.107");
    ...
    * @var string
    */
	//protected $adminUsername = "manager";
    ...
    */ 
    ...
    /** 
    * Convert time stored in AD tr real Time
    * created by m.ezzairi
    * for great time format change in time format (Y-m-d) 
    * @param windowsTime from AD like lastlogontimestamp
    * @return bool
    */ 
    public function convertTime($windowsTime=null) {
        if($windowsTime!=null){
            $windowsTime=adLDAPUtils::convertWindowsTimeToUnixTime($windowsTime);
            $windowsTime=date('d/m/Y H:i:s',$windowsTime);
            return $windowsTime;
        }
        return 0;
    }
    ...
    /**
    * Convert DN to container like exemple.com/Devlopper/CackePHP
    * created by m.ezzairi
    * @param string object DN from AD
    * @return string container
    */
    public function getContainer($dn=null) {
        if($dn!=null){
            $dn=substr($dn, 3); //delete CN=
            $dn=substr($dn, strpos($dn, ',')); //delete Objet name
            $dn=substr($dn, 0,strlen($dn)-strlen($this->getBaseDn())-1); //delete dn Base
            $dnArray=explode(',OU=', $dn); //explode to array
            $dnArray[0]=substr($dn, 1,3)=='CN=' ? substr($dn,4) :$dnArray[0]; //delete CN= for builtin
            $dnArray=array_reverse($dnArray); //reverse the array
            $dn=implode('/', $dnArray); //implode to straing with / separator
            $dn=strtoupper($this->getAccountSuffix()).'/'.$dn; //add the @exemple.com/ to began
            $dn=substr($dn, 1); //delete the @at the start of the DN
            return $dn;
        }
        return 0;
    }
    ...
    /**
    * Get the name for an object from its DN
    * created by m.ezzairi
    * @param string object DN from AD
    * @return string name
    */
    public function getName($dn=null) {
        if($dn!=null){
            $dn=substr($dn, 3); //delete CN=
            $dn=substr($dn,0, strpos($dn, ',')); //get Objet name
            return $dn;
        }
        return 0;
    }

...
class adLDAPGroups {
    ...
    */
    public function create($attributes)
    {
        ...
        if (!array_key_exists("grouptype", $attributes)){ $attributes["grouptype"]=-2147483644; }
        ...
        $add["grouptype"][0]    = $attributes["grouptype"];
        if (array_key_exists("description",$attributes) && !empty($attributes['description'])){
            $add["description"] = $attributes['description'];
        }
        $container =$attributes["container"];
        $result = ldap_add($this->adldap->getLdapConnection(), "cn=" . $add["cn"] . "," . $container, $add);
        if ($result != true) { 
            return false; 
        }
        return true;
    ...
    */
    public function search($sAMAaccountType = adLDAP::ADLDAP_SECURITY_GLOBAL_GROUP, $includeDescription = false, $search = "*", $sorted = true) {
        ...
        /*
        $filter = '(&(objectCategory=group)(samaccounttype=*)';
    ...
    */
    public function all($search = "*"){
        $groups = $this->search(null,false,$search);
        for ($i=0; $i<$groups["count"]; $i++){
            $groups[$i]['name']     =$groups[$i]['samaccountname'][0];
            $groups[$i]['total']    =isset($groups[$i]['member']["count"]) ? $groups[$i]['member']["count"]:0;
            $groups[$i]['container']=$this->adldap->getContainer($groups[$i]['distinguishedname'][0]);
            $groups[$i]['type']     =$this->getGroupType($groups[$i]['grouptype'][0]);
            $groups[$i]['links']    ='<a href="?cat=group&do=infos&group='.$groups[$i]['samaccountname'][0].'">Infos</a>';
        }
        return $groups;
    }
    ...
    * @return bool
    */
    public function move($group, $container) 
    {
        if (!$this->adldap->getLdapBind()) { return false; }
        if ($group === null) { return false; }
        if ($container === null) { return false; }

        $groupInfo = $this->all($group);
        $dn = $groupInfo[0]['distinguishedname'][0];
        $newRDn = "cn=" . $group;
    ...
    /**
    * get the group type
    *@param string $gouptype
    *@return string
    */
    public function getGroupType($gouptype)
    {
        if ($gouptype===NULL) {return false;}
        if ($gouptype==-2147483644) {return "Securité Domaine Local";}
        if ($gouptype==-2147483646) {return "Securité Global";}
        if ($gouptype==-2147483640) {return "Security Universel";}
    }
    /**
    * get the groups which are possible contenant(be parents) for a group
    *@param string $group
    *@return array $groups
    */
    public function getPossibleGroups($group)
    {
        
        if ($group===NULL) {return false;}
        $group=$this->all($group);
        $group=$group[0];
        $groupName=$group["samaccountname"][0];
        $groupType=$group["grouptype"][0];
        $memberOf=array();
        if(array_key_exists('memberof',$group)){
            for ($i=0;$i<$group['memberof']['count'];$i++) {
                $memberName=$this->adldap->getName($group['memberof'][$i]);
                array_push($memberOf, $memberName);
            }
        }
        $allGroups=$this->all();
        if(!empty($allGroups)){
            $rien=array_shift($allGroups);
            $groupPossible=array();
            foreach ($allGroups as $key => $curGroup) {
                //the current group name
                if($groupName==$curGroup["samaccountname"][0]) continue;
                //if is already member of the current group
                if(in_array($curGroup["samaccountname"][0], $memberOf)) continue;
                if($groupType==ADLDAP::LOCAL_GROUP){//Local
                    if($curGroup["grouptype"][0]==ADLDAP::LOCAL_GROUP){
                        $groupPossible[]=$curGroup;
                    }
                }else if($groupType==ADLDAP::UNIVERSEL_GROUP){//Universel
                    if($curGroup["grouptype"][0]!=ADLDAP::GLOBAL_GROUP){
                        $groupPossible[]=$curGroup;
                    }
                }else{//Global
                    $groupPossible[]=$curGroup;
